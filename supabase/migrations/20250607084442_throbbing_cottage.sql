/*
  ...
  1. New Tables
    - `users` - User management with credit system (250 welcome credits)
    - `service_types` - Available Docker services (n8n, Chatwoot, PostgreSQL)
    - `services` - User deployed services with Docker configuration
    - `credit_transactions` - Credit usage and refund tracking
    - `user_addons` - Premium add-ons (custom domains, SSL, etc.)
    
  2. Security
    - Enable RLS on all tables
    - Users can only access their own data
    - Admins can manage service types
    - Secure user profile creation trigger
    
  3. Features
    - Auto-generate user profile on signup with 250 credits
    - 3 pre-configured Docker services ready to deploy
...
-- Create utility function for updating user credits
