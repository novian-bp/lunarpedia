/*
  ...
  2. Security
    ...
    - Admin-only access for service type management
...
-- Drop existing objects in correct order (triggers first, then functions, then tables)
DO $$ 
BEGIN
  -- Drop triggers first
  ...
  DROP FUNCTION IF EXISTS calculate_monthly_usage(uuid) CASCADE;
  ...
  DROP POLICY IF EXISTS "service_types_select_published" ON service_types;
  ...
  DROP POLICY IF EXISTS "user_addons_insert_own" ON user_addons;
  DROP POLICY IF EXISTS "user_addons_update_own" ON user_addons;
...
CREATE POLICY "service_types_select_published"
  ON service_types
  FOR SELECT
  TO authenticated
  USING (status = 'published');

CREATE POLICY "service_types_admin_all"
  ON service_types
  FOR ALL
  TO authenticated
  USING (
